...
def estimate_noise_sigma(pc: np.ndarray, k: int = 16) -> float:
    """局部 PCA 法向残差估计单轴噪声 std（与 estimate_noise_level.py 同原理）。"""
    from scipy.spatial import cKDTree

    pts = pc.astype(np.float64)
...
def parse_alpha_buckets(spec: str):
    """'0.010:0.5,0.020:0.8' → [(0.010, 0.5), (0.020, 0.8)]，按阈值升序。

    语义：估计噪声 < 阈值时使用对应 alpha；超过所有阈值则 alpha=1.0。
    用于保守混合：低噪声样本模型修正不可靠，缩小位移可避免 clamp 0 分。
    """
    buckets = []
    for item in spec.split(','):
        thr, alpha = item.split(':')
        buckets.append((float(thr), float(alpha)))
    return sorted(buckets)


def alpha_for_sigma(sigma: float, buckets) -> float:
    for thr, alpha in buckets:
        if sigma < thr:
            return alpha
    return 1.0
...
def main():
    ...
    parser.add_argument("--blend_alpha", type=float, default=1.0,
                        help="全局保守混合系数：output = noisy + alpha*(denoised-noisy)，1=不混合")
    parser.add_argument("--alpha_buckets", default="",
                        help="按估计噪声分桶的 alpha，如 '0.010:0.5,0.020:0.8'（覆盖 blend_alpha）")
    ...
    for index, lazy_asset in enumerate(assets, start=1):
        ...
        # 保守混合：低噪声样本模型修正不可靠时，缩小位移量
        alpha = args.blend_alpha
        if args.alpha_buckets:
            sigma = estimate_noise_sigma(noisy)
            alpha = alpha_for_sigma(sigma, parse_alpha_buckets(args.alpha_buckets))
        if alpha < 1.0:
            fused = noisy + alpha * (fused - noisy)
